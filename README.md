...
1. Explain the difference between Relational Databases and SQL.
    1. A relational database is a data storage system and SQL is the language used to interact with that system.
2. Why do tables need a Primary Key?
    1. A primary key is a unique identifier for a record. It is necessary to ensure data integrity on queries and cross-references (i.e. foreign key relationships).
3. What is the name given to a table column that references the Primary Key on another table?
    1. This is a foreign key. Specifically, the name should match the other table name followed by "id".
4. What do we need in order to have a _many to many_ relationship between two tables?
    1. An intermediate table that includes a unique identifier (primary key) and two foreign keys linking the two tables with the many-to-many relationship.
